...
1. [Releases](https://github.com/tjeodjq/welaaa/releases)에서 `welaaa-1.1.20.zip`을 받습니다. 비공개 저장소는 Git URL 설치가 되지 않으므로 ZIP을 사용하세요.
...
비디오북 Health/카테고리의 「메타·포스터 새로고침」은 제목 검색 없이 `link`의 web_id만 사용합니다. `https://www.welaaa.com/content/{id}` 는 비디오북에서 `https://www.welaaa.com/video/detail/{id}` 로 바꿉니다. 클래스 가로 배너는 `new_images` klass-cover를 쓰고, 403이 나는 `/static/courses/*_wide.jpg` 는 건너뜁니다. CDN이 실패하면 폴더의 가로 poster만 사용합니다.
