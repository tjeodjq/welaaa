...
1. [Releases](https://github.com/tjeodjq/welaaa/releases)에서 `welaaa-1.1.22.zip`을 받습니다. 비공개 저장소는 Git URL 설치가 되지 않으므로 ZIP을 사용하세요.
...
비디오북 Health/카테고리의 「메타·포스터 새로고침」과 수동 검색 적용은 제목 검색 없이 `link`의 web_id로 상세를 가져옵니다. 적용 결과는 DB뿐 아니라 폴더 윌라 JSON(`cover_url`, `link`, 저자·소개)과 가로 `poster.jpg` 에도 덮어씁니다. `https://www.welaaa.com/content/{id}` 는 `https://www.welaaa.com/video/detail/{id}` 로 바꿉니다. 클래스 가로 배너는 `new_images` klass-cover를 쓰고, 403이 나는 `/static/courses/*_wide.jpg` 는 건너뜁니다.
