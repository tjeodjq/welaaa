...
1. [Releases](https://github.com/tjeodjq/welaaa/releases)에서 `welaaa-1.1.19.zip`을 받습니다. 비공개 저장소는 Git URL 설치가 되지 않으므로 ZIP을 사용하세요.
...
비디오북 Health/카테고리의 「메타·포스터 새로고침」은 제목 검색 없이 `link`의 web_id만 사용합니다. `https://www.welaaa.com/content/{id}` 는 비디오북에서 `https://www.welaaa.com/video/detail/{id}` 로 바꿉니다. 오디오 상세(`/audio/detail`)로 가지 않습니다. 기본은 표지만 16:9 캔버스에 **여백으로 맞춤**(크롭 없음)으로 다시 받고 `metadata_locked`는 유지합니다. 검색 결과의 세로 `list` 썸네일로 덮지 않습니다.
